package com.tujia.train.mybatis.mapperProvider;

import com.tujia.framework.utility.StringUtils;
import com.tujia.train.mybatis.Dao.BaseDaoImpl;
...
import com.tujia.train.mybatis.core.JoinNode;
...
public class BaseProvider {
    ...
    private void joinField(StringBuffer buffer, String[] likeNames, String paramName, String inParam) {
        ...
        if (likeNames != null && ArrayUtils.contains(likeNames, paramName)) {
            ...
            buffer.append("},'%')");
        } else if (StringUtils.isNotBlank(inParam)) {
            buffer.append(" in");
    ...
    private void tableName(ProviderContext context) {
        ...
        if (Objects.isNull(metaMethod) || StringUtils.isBlank(metaMethod.tableName())) {
    ...
    public static void main(String[] args) throws ClassNotFoundException {
      /*  String helloWorld = fieldToUnderCamelCase("helloWorldMing");
        ...
        Class<?> returnType = Class.forName("com.tujia.train.mybatis.po.News");
        Field[] declaredFields = returnType.getDeclaredFields();
        StringBuffer buffer = new StringBuffer();
        for (int i = 1; i < declaredFields.length; i++) {
            buffer.append( fieldToUnderCamelCase(declaredFields[i].getName()));
