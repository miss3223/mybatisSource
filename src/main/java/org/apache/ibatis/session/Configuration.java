...
public class Configuration {
  ...
  /** 不允许自定义ResultHandler，如果允许则设置false */
  ...
  * 懒加载
   ...
   *  这就是insert时获取mysql自增主键/oracle sequence的开关。
 ...
 * MyBatis 可以根据不同的数据库厂商执行不同的语句，这种多厂商的支持是基于映射语句中的 databaseId 属性。
  * */
  ...
  public Configuration() {
    ...
    //数据源类型别名
    /*
    * NDI：Mybatis从在应用服务器向配置号的JNDI数据源dataSource获取数据库连接。在生产环境中，优先考虑这种方式。
    * */
    typeAliasRegistry.registerAlias("JNDI", JndiDataSourceFactory.class);
    /*
    *POOLED：创建了一个数据库连接池，连接池中的一个连接将会被用作数据库操作。一旦数据库操作完成，Mybatis会将此链接返回给连接池。
    * 在开发和测试环境中，经常使用这种方式。
    * */
    typeAliasRegistry.registerAlias("POOLED", PooledDataSourceFactory.class);
    /**
     * UNPOOLED：Mybatis会为每一个数据库操作创建一个新的连接，并关闭它。该方式适用于只有小规模数据并发用户的简单应用上。
     */
  ...
  public Executor newExecutor(Transaction transaction, ExecutorType executorType) {
    ...
    // 缓存则用CachingExecutor对象进行包装Executor
    if (cacheEnabled) {
  ...
  public void addResultMap(ResultMap rm) {
    ...
    // 若传入的ResultMap不存在内嵌ResultMap并且有Discriminator，则判断是否需要强制表位有内嵌的 ResultMap
  ...
  /** 扫描该包下的所有Mapper接口，并添加到mapperRegistry中 */
