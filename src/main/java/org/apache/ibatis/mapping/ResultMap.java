...
public class ResultMap {
  ...
  /**
   * ID ResultMapping 集合
   * 当 idResultMappings为空时，使用resultMappings赋值
   * */
   ...
   * 是否开启自动匹配
   *
   * 如果设置了这个属性，Mybatis将会为这个ResultMap开启或者关闭自动映射。
   * 这个属性会覆盖全局的属性 autoMappingBehavior。默认值是：unset
   *
   */
  ...
  public static class Builder {
    ...
    public ResultMap build() {
      ...
      for (ResultMapping resultMapping : resultMap.resultMappings) {
        ...
        // 初始化constructorResultMappings
        if (resultMapping.getFlags().contains(ResultFlag.CONSTRUCTOR)) {
          resultMap.constructorResultMappings.add(resultMapping);
          if (resultMapping.getProperty() != null) {
            constructorArgNames.add(resultMapping.getProperty());
          }
          // 初始化propertyResultMappings
        } else {
      ...
      // 将constructorResultMappings排序成符合的构造方法的参数顺序
